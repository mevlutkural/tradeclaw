...
function RankBadge({ rank }: { rank: number }) {
  ...
  if (rank === 2)
    return (
      <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[var(--glass-bg)] text-[var(--text-secondary)] text-[10px] font-bold">
        2
      </span>
    );
  if (rank === 3)
...
function HitRateBar({ value, size = 'md' }: { value: number; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={`relative flex-1 ${h} rounded-full bg-[var(--border)]`}>
        <div
...
function ConfBar({ value }: { value: number }) {
  return (
    <div className="flex items-center gap-2">
      <div className="relative flex-1 h-[3px] rounded-full bg-[var(--border)]">
        <div
          className="absolute h-[3px] rounded-full bg-[var(--text-secondary)] transition-all duration-700"
          style={{ width: `${value}%` }}
        />
      </div>
          ...
          <div className="flex-1 overflow-y-auto">
                ...
                <table className="w-full min-w-[420px]">
                  ...
                  <tbody>
                    {data.records.slice(0, 20).map(r => (
                      <tr key={r.id} className="border-b border-[var(--border)] hover:bg-[var(--glass-bg)]">
                        ...
                        <td className="px-3 py-2 text-right text-[10px] font-mono text-[var(--foreground)]">{fmtPrice(r.entryPrice)}</td>
            ...
            <div className="px-4 pb-4">
              <a
                ...
                className="inline-flex items-center gap-2 px-3 py-2 text-[10px] text-[var(--text-secondary)] hover:text-[var(--foreground)] bg-[var(--glass-bg)] border border-[var(--border)] rounded-lg transition-colors"
              >
    ...
    <div className="min-h-[100dvh] bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 pb-20 md:pb-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          {/* Period */}
          <div className="flex gap-1 bg-[var(--glass-bg)] rounded-xl p-1 border border-[var(--border)]">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-[10px] text-[var(--text-secondary)] hover:text-[var(--foreground)] bg-[var(--glass-bg)] border border-[var(--border)] rounded-lg transition-colors"
          >
          ...
          <table className="w-full min-w-[640px]">
            <thead>
              <tr className="border-b border-[var(--border)]">
                ...
                <th
                  className="px-4 py-3 text-[10px] text-[var(--text-secondary)] uppercase tracking-wider font-medium w-32 cursor-pointer hover:text-[var(--foreground)] select-none"
            ...
            <tbody>
              ...
              {!loading && assets.map((asset, idx) => (
                <tr
                  key={asset.pair}
                  className="border-b border-[var(--border)] hover:bg-[var(--glass-bg)] transition-colors cursor-pointer"
