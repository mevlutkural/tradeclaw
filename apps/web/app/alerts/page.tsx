...
function CreateAlertModal({ onClose, onCreated, prefillSymbol, prefillPrice }: CreateAlertModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-[var(--background)]/80 backdrop-blur-sm">
      <div className="w-full max-w-md bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-semibold text-[var(--foreground)]">New Price Alert</h2>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-[var(--glass-bg)] text-[var(--text-secondary)] hover:bg-[var(--border)] transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                ...
                className={`py-2.5 rounded-lg text-sm font-semibold border transition-colors ${
                  direction === 'above'
                    ...
                    : 'bg-[var(--glass-bg)] text-[var(--text-secondary)] border-[var(--border)] hover:border-[var(--glass-border-accent)]'
                }`}
              >
                ▲ Above
              </button>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[var(--glass-bg)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--foreground)] font-mono tabular-nums focus:outline-none focus:border-emerald-500/50 transition-colors"
              required
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="e.g. 2.5"
                className="w-full bg-[var(--glass-bg)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--foreground)] font-mono focus:outline-none focus:border-emerald-500/50 transition-colors"
              />
            </div>
            <div>
    ...
    <div
      className={`rounded-xl border p-4 transition-all ${
        ...
        isTriggered
          ...
          : 'border-[var(--border)] bg-[var(--glass-bg)] hover:border-[var(--glass-border-accent)]'
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 text-[10px] text-[var(--text-secondary)] opacity-60 font-mono">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {!isTriggered && (
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-lg bg-[var(--glass-bg)] text-[var(--text-secondary)] hover:text-[var(--foreground)] hover:bg-[var(--border)] transition-colors"
            >
      ...
      <div className="w-full max-w-md bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-[10px] text-[var(--text-secondary)] uppercase tracking-wider mb-1.5 block">Direction</label>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setDirection('above')} className={`py-2.5 rounded-lg text-sm font-semibold border transition-colors ${direction === 'above' ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' : 'bg-[var(--glass-bg)] text-[var(--text-secondary)] border-[var(--border)]'}`}>▲ Above</button>
              <button type="button" onClick={() => setDirection('below')} className={`py-2.5 rounded-lg text-sm font-semibold border transition-colors ${direction === 'below' ? 'bg-rose-500/15 text-rose-400 border-rose-500/30' : 'bg-[var(--glass-bg)] text-[var(--text-secondary)] border-[var(--border)]'}`}>▼ Below</button>
            </div>
          </div>
          <div>
          ...
          <div>
            <label className="text-[10px] text-[var(--text-secondary)] uppercase tracking-wider mb-1.5 block">Note (opt)</label>
            <input type="text" value={note} onChange={e => setNote(e.target.value)} maxLength={100} className="w-full bg-[var(--glass-bg)] border border-[var(--border)] rounded-lg px-3 py-2.5 text-sm text-[var(--foreground)] focus:outline-none focus:border-emerald-500/50 transition-colors" />
          </div>
        ...
        {stats && (
          <div className="grid grid-cols-3 gap-2 mb-6">
            {[
              ...
              <div key={label} className="bg-[var(--glass-bg)] border border-[var(--border)] rounded-xl px-3 py-3 text-center">
          ...
          <>
            ...
            <section className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-xs text-[var(--text-secondary)] uppercase tracking-wider font-semibold">
                  Active <span className="opacity-60">({activeAlerts.length})</span>
                </h2>
              </div>
              {activeAlerts.length === 0 ? (
            ...
            {triggeredAlerts.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-xs text-[var(--text-secondary)] uppercase tracking-wider font-semibold">
                    Triggered <span className="opacity-60">({triggeredAlerts.length})</span>
                  </h2>
                </div>
                <div className="space-y-2">
